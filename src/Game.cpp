...
Game::Game()
    : windowManager(Constants::SIZE_X, Constants::SIZE_Y, "Asteroids"),
      lives(5), score(0), paused(false), bulletCooldown(0.05f), gameState(GameState::MENU),
      menu(Constants::SIZE_X, Constants::SIZE_Y), settings(Constants::SIZE_X, Constants::SIZE_Y) {
...
void Game::spawnAsteroid(int size, const std::optional<sf::Vector2f>& position, bool checkSpaceshipPosition) {
    ...
    while (!validPosition) {
        finalPosition = position.value_or(sf::Vector2f(
            Utils::getRandomInRange(0.f, windowManager.getWindow()->getSize().x),
            Utils::getRandomInRange(0.f, windowManager.getWindow()->getSize().y)));

        if (checkSpaceshipPosition) {
            float dx = finalPosition.x - gameObjectManager.getSpaceship()->sprite.getPosition().x;
            float dy = finalPosition.y - gameObjectManager.getSpaceship()->sprite.getPosition().y;
            float distance = std::sqrt(dx * dx + dy * dy);

            if (distance > gameObjectManager.getSpaceship()->getRadius() * 3) {
                validPosition = true;
            }
        } else {
            validPosition = true;
    ...
    sf::Vector2f velocity = gameObjectManager.generateRandomVelocity();
    gameObjectManager.getAsteroids().emplace_back(resourceManager.getAsteroidTexture(), size, finalPosition, velocity);
}
